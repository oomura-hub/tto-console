...
async function runGenerator() {
    ...
    try {
        ...
        const fmtTemplates = {
            '二項対立': `
【出力形式（二項対立FMT・厳守）】
各パターンは以下の形式で出力すること。1枚目はHook、2〜8枚目は二項対立スライドとする。
...
**・1枚目（Hook）**
- テキスト：（キャッチコピー。例：「非モテが加速するNG習慣7選」）
- 画像：（どんな写真を使うか。マニュアル基準に沿って具体的に）

**・2枚目**
- タイトル：（このスライドのテーマ。例：「趣味」）
- NG：（NG側のラベル。例：「ジム」）
- NG説明：（なぜNGかの理由。3行以内で簡潔に）
- ◯：（正解側のラベル。例：「クラブ」）
- ◯説明：（なぜ◯かの理由。3行以内で簡潔に）

**・3枚目〜8枚目**（2枚目と同じ形式で繰り返す）
- タイトル：
- NG：
- NG説明：
- ◯：
...
### 【各内容の選定理由】
- 1枚目：（なぜこの画像・テキストにしたか）
- 2枚目：（なぜこのテーマで、なぜこのNG/◯にしたか。常識破壊の度合い）
- 3枚目以降：（滞在時間・離脱防止の設計意図）
---`,
...
---`
        ...
        // FMTに応じた出力形式を選択（autoや未定義は汎用）
        const fmtOutput = fmtTemplates[fmt] || `
【出力形式（厳守）】
各パターンは以下の形式で出力すること：
...
${fmtOutput}`;
        ...
        if (url) {
            const fmtLabel = fmt === 'auto' ? '動画に最適な形式を判定して使用' : `「${fmt}」形式で出力`;
...
2. その分析に基づき、上記の【出力形式】に従って新しい構成案を3パターン（Pattern A, B, C）作成してください。
3. 各パターンは必ず1枚目〜8枚目まで、スライドごとに指定された項目を全て記載してください。
4. 最後に【各内容の選定理由】を記載し、なぜその構成にしたかをマニュアルの原則に紐づけて解説してください。`;
